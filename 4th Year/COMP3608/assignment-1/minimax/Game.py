import math
from Board import *
from utility import UTILITY, EVALUATION
...
class Game:
    ...
    def make_move(self, col):
        if not 0 <= col < self.board.N_COLS or self.board.get_pos(0, col) != '.':
            return False
        return self.board.place_token_col(col, self.CURRENT_TURN)    
    ...
    def minimax(self, curr_depth, max_depth, curr_turn, is_maximising):
        ...
        if curr_depth == max_depth:
            ...
            #print(f"DEPTH REACHED, eval = {evaluation}")
            return evaluation, total_nodes_examined
        ...
        if is_maximising:
            ...
            for move in possible_moves:
                self.board.place_token(move[0], move[1], curr_turn)
                
                score, nodes_examined = self.minimax(curr_depth + 1, max_depth, self.get_next_turn(curr_turn), False)
                total_nodes_examined += nodes_examined 
                
                self.board.unplace_token(move[0], move[1], curr_turn)
            ...
            for move in possible_moves:
                ...
                score, nodes_examined = self.minimax(curr_depth + 1, max_depth, self.get_next_turn(curr_turn), True)
                total_nodes_examined += nodes_examined
    ...
    def get_best_move(self, max_depth, player):
        if (player == "r"):
            best_score = -math.inf
        elif (player == "y"):
            best_score = math.inf
        ...
        for move in possible_moves:
            self.board.place_token(move[0], move[1], player)

            #print(f"Entering minimax on move {move}")
            score, nodes_examined = self.minimax(1, max_depth, self.get_next_turn(player), False)
            total_nodes_examined += nodes_examined

            #print(f"score for move {move} = {score}")
            #print()
            
            self.board.unplace_token(move[0], move[1], player)
    ...
    def play(self):
        ...
        while True:
            ...
            if self.CURRENT_TURN == "r":
                ...
                best_move, total_nodes_examined = self.get_best_move(max_depth=1, player="y")
